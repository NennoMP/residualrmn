...
DATA_DIR = "../data"  # directory where datasets should be stored
...
parser.add_argument(
    "--batch_size", type=int, default=1024, help="mini-batch size (default: 1024)"
)
parser.add_argument(
...
# Hyperparameters
parser.add_argument(
    "--skip_option_t", type=str, help="temporal residual connections configuration"
)
parser.add_argument(
    "--n_units_m", type=int, default=100, help="memory reservoir units (default: 100)"
)
parser.add_argument(
    "--n_units", type=int, default=100, help="non-linear reservoir units (default: 100)"
)
parser.add_argument(
    "--in_scaling_m",
    type=float,
    default=1.0,
    help="memory reservoir input kernel scaling (default: 1.0)",
)
parser.add_argument(
    "--bias_scaling_m",
    type=float,
    default=0.0,
    help="memory reservoir bias vector scaling (default: 0.0)",
)
parser.add_argument(
    "--in_scaling",
    type=float,
    default=1.0,
    help="non-linear reservoir input kernel scaling (default: 1.0)",
)
parser.add_argument(
    "--memory_scaling",
    type=float,
    default=1.0,
    help="non-linear reservoir memory kernel scaling (default: 1.0)",
)
parser.add_argument(
    "--bias_scaling",
    type=float,
    default=0.0,
    help="non-linear reservoir bias vector scaling (default: 0.0)",
)
parser.add_argument(
    "--rho", type=float, default=1.0, help="spectral radius (default: 1.0)"
)
parser.add_argument(
    "--alpha",
    type=float,
...
parser.add_argument(
    ...
    type=float,
    default=0.0,
    help="readout regularization strength (default: 0.0)",
)
...
def main() -> None:
    ...
    if args.dataset == "psMNIST":
        full_train_dataloader, _, _, test_dataloader = data_utils.get_mnist(
            data_dir=f"{DATA_DIR}", batch_size=args.batch_size
        )
        # Load permutation
        with open(f"{DATA_DIR}/MNIST/permutation.json") as inf:
            permutation = json.load(inf)
    else:
        full_train_dataloader, _, _, test_dataloader = (
            data_utils.get_classification_task(
