...
import torch
import torchvision
from aeon.datasets import load_from_ts_file, load_from_arff_file
...
def get_classification_task(
    ...
    """
    # Load training data
    if os.path.exists(f'{data_dir}/{dataset_name}_TRAIN.arff'):
        train_dataset, train_target = load_from_arff_file(f'{data_dir}/{dataset_name}_TRAIN.arff')
    elif os.path.exists(f'{data_dir}/{dataset_name}_TRAIN.ts'):
        train_dataset, train_target = load_from_ts_file(f'{data_dir}/{dataset_name}_TRAIN.ts')
    else:
        raise ValueError(f"Training data for {dataset_name} not found in {data_dir}. Make sure to download the dataset from https://www.timeseriesclassification.com/ and place it in the appropriate directory.")
    # Load test data
    if os.path.exists(f'{data_dir}/{dataset_name}_TEST.arff'):
        test_dataset, test_target = load_from_arff_file(f'{data_dir}/{dataset_name}_TEST.arff')
    elif os.path.exists(f'{data_dir}/{dataset_name}_TEST.ts'):
        test_dataset, test_target = load_from_ts_file(f'{data_dir}/{dataset_name}_TEST.ts')
    else:
        raise ValueError(f"Test data for {dataset_name} not found in {data_dir}. Make sure to download the dataset from https://www.timeseriesclassification.com/ and place it in the appropriate directory.")
