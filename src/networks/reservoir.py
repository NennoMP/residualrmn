"""Module containing PyTorch implementations of various reservoir modules employed in
ResRMN.

Note that the concept of a linear memory reservoir has been introduced in [1], and the
concept of residual echo state networks has been introduced in [2].

References:
[1] C. Gallicchio and A. Ceni, Reservoir Memory Networks, ESANN (2024).
[2] A. Ceni and C. Gallicchio, "Residual Echo State Networks: Residual recurrent neural
networks with stable dynamics and fast learning." Neurocomputing (2024).
"""
...
class MemoryCell(nn.Module):
    ...
    The memory reservoir applies a linear transformation and is driven solely by the
    external input. The recurrent kernel is initialized as a cyclic (circular shift)
    orthogonal matrix.
    """
...
class ReservoirCell(nn.Module):
    ...
    The non-linear (residual) reservoir applies a non-linear transformation and is
    driven by the external input and the output of the memory reservoir. Its state
    update function consists of a linear branch (temporal residual connections) and a
    non-linear branch. The recurrent kernel is initialized as a random matrix and then
    rescaled to have a desired spectral radius.
    """
    ...
    def forward(
        self, x: torch.Tensor, m: torch.Tensor, h_prev: torch.Tensor
    ) -> torch.Tensor:
        input_part = torch.mm(x, self.in_kernel)
        ...
        out = (self.alpha * residual_part) + (
            self.beta * self.act(state_part + memory_part + input_part + self.bias)
        )
        return out


class Reservoir(nn.Module):
    def __init__(self, cell: MemoryCell | ReservoirCell) -> None:
        """
        Args:
            cell: The reservoir cell to be used in the reservoir. Options are
            'MemoryCell' or a 'ReservoirCell'.

        Raises:
            AssertionError: If the cell is not an instance of 'MemoryCell' or
            'ReservoirCell'.
        """
        super().__init__()
    ...
    def forward(
        ...
        Returns:
            The hidden statest of the model, with shape (batch_size, T, n_units), and
            the hidden state at the last time step, with shape (batch_size, n_units).
        """
